...
def limpiar_sex(df): 
    ...
    """
    df['Sex'] = df['Sex'].apply(lambda x: "M" if x == "M " else x)
    df['Sex'] = df['Sex'].apply(lambda x: "Unknown" if x == "N" else x)
    df['Sex'] = df['Sex'].apply(lambda x: "Unknown" if x == "lli" else x)
    df['Sex'] = df['Sex'].apply(lambda x: "Unknown" if x == "." else x)
    df['Sex'] = df['Sex'].apply(lambda x: "Unknown" if x == "UNKNOWN" else x)
    return df.Sex
...
def limpiar_area(dfCopy):
    """
    Esta función elimina duplicados y corrige errores 
    gramaticales o de puntuación en Area
    
    """
    dfCopy.Area = dfCopy.Area.fillna("Unknown")
    dfCopy.Area = dfCopy.Area.str.replace("Victoria ", "Victoria")
    dfCopy.Area = dfCopy.Area.str.replace("Torres Strait", "Queensland")
    dfCopy.Area = dfCopy.Area.str.replace("Torres Strait ", "Queensland")
    dfCopy.Area = dfCopy.Area.str.replace("Queensland ", "Queensland")
    dfCopy.Area = dfCopy.Area.str.replace("Between Timor & Darwin, Australia", "Northern Territory")
    dfCopy.Area = dfCopy.Area.str.replace("Westerm Australia", "Western Australia")
    dfCopy.Area = dfCopy.Area.str.replace("Between Timor and Darwin, Australia", "Northern Territory")
    return dfCopy.Area
